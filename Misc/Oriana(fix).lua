...
function OnLoad()
	...
	Menu:addSubMenu("KillSteal", "KillSteal")
		Menu.KillSteal:addParam("KsEnable", "Smart KillSteal", SCRIPT_PARAM_ONOFF , true)
		...
		Menu.KillSteal:addParam("Debug", "print chat ks", SCRIPT_PARAM_ONOFF, true)	
	...
	Menu:addSubMenu("Drawing", "Drawing")
		...
		DManager:CreateCircle(myHero, Qrange, 3, {255, 255, 255, 255}):AddToMenu(Menu.Drawing, "Q Range", true, true, true)
		Circle_W = DManager:CreateCircle(myHero, Wradius, 3, {255, 255, 255, 255})
			Circle_W:AddToMenu(Menu.Drawing, "W Range", true, true, true)
		DManager:CreateCircle(myHero, Erange, 3, {255, 255, 255, 255}):AddToMenu(Menu.Drawing, "E Range", true, true, true)
...
function CastI(target)
	if GetDistanceSqr(target) <= 360000  then
...
function Farm(Mode)
	...
	if UseQ and QREADY then
		if UseW then
			...
			for i, minion in pairs(EnemyMinions.objects) do
				if minion.health + 15 < GetDamage(_Q, minion) and not ValidTarget(minion, SxOrb.MyRange) then
					local MinionPos = VP:GetPredictedPos(minion, Qdelay, BallSpeed, BallPos)
	...
	for i=1, heroManager.iCount do
		...
		if ValidTarget(enemy, Qrange+Rradius/2)then
			...
			if health < GetComboDamage({_Q}, enemy) and QREADY and Menu.KillSteal.UseQ then
				if CastQ(enemy) then if Menu.KillSteal.Debug then PrintChat(enemy.charName.." "..os.clock().." KS activate Q") end return end
			elseif health < GetComboDamage({_W}, enemy) and WREADY and Menu.KillSteal.UseW then
				if CastW(enemy) then if Menu.KillSteal.Debug then PrintChat(enemy.charName.." "..os.clock().." KS activate W") end return end
			elseif health < GetComboDamage({_Q, _W}, enemy)  and QREADY and WREADY and Menu.KillSteal.UseQ and Menu.KillSteal.UseW then
				if CastQ(enemy) then if Menu.KillSteal.Debug then PrintChat(enemy.charName.." "..os.clock().." KS activate Q , W") end return end
			elseif health < GetComboDamage({_R}, enemy) and RREADY and Menu.KillSteal.UseR then
				if CastR(enemy) then if Menu.KillSteal.Debug then PrintChat(enemy.charName.." "..os.clock().." KS activate R") end return end
			elseif health < GetComboDamage({_Q, _R}, enemy)  and QREADY and WREADY and Menu.KillSteal.UseQ and Menu.KillSteal.UseR then
				if CastQ(enemy) then if Menu.KillSteal.Debug then PrintChat(enemy.charName.." "..os.clock().." KS activate Q , R") end return end
			elseif health < GetComboDamage({_W, _R}, enemy)  and WREADY and RREADY and Menu.KillSteal.UseW and Menu.KillSteal.UseR then
				if CastR(enemy) then if Menu.KillSteal.Debug then PrintChat(enemy.charName.." "..os.clock().." KS activate R , W") end end
			elseif health <= GetComboDamage({_Q, _W, _R}, enemy) and QREADY and WREADY and RReady and Menu.KillSteal.UseQ and Menu.KillSteal.UseW and Menu.KillSteal.UseR then
				if CastQ(enemy) then if Menu.KillSteal.Debug then PrintChat(enemy.charName.." "..os.clock().." KS activate Q , W , R") end return end
			elseif health < GetComboDamage({_IGNITE}, enemy) and IGNITEREADY and Menu.KillSteal.UseI then
				if CastI(enemy)then if Menu.KillSteal.Debug then PrintChat(enemy.charName.." "..os.clock().." KS activate IGNITE") end return end 
			elseif health < GetComboDamage({_IGNITE, _Q}, enemy) and IGNITEREADY and QREADY and Menu.KillSteal.UseI and Menu.KillSteal.UseQ then
				if CastI(enemy)then if CastQ(enemy) then if Menu.KillSteal.Debug then PrintChat(enemy.charName.." "..os.clock().." KS activate IGNITE , Q") end return end end
			end
		end
	end
end
...
function OnTick()
	...
	if Menu.KillSteal.KsEnable then
		KillSteal()
	end
...
function DrawOnHPBar(unit, health)
	...
	if health < 0 then
		DrawText("HP: "..health,13, Position, SPos.y, ARGB(255,255,0,0))
	else
		DrawText("HP: "..health,13, Position, SPos.y, ARGB(255,0,255,0))
	end
